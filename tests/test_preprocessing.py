...
- Đảm bảo person bị sai acquisition protocol có thể được loại đúng cách.
...
def valid_df() -> pd.DataFrame:
    ...
    - đủ 4 labels;
    - có person01;
    - có person05 để test protocol filtering;
    - đúng toàn bộ feature columns từ FeatureExtractor V2;
    - không có NaN / Inf / duplicate.

    Mỗi class được tạo cho cả person01 và person05.
    Nhờ vậy sau khi loại person05, dataset vẫn còn đủ 4 class.
    """
...
def test_filter_protocol_invalid_loai_person05(
    valid_df: pd.DataFrame,
) -> None:
    """
    Khi excluded_persons có person05,
    output không được còn sample nào của person05.
    """
    ...
    # Sau filter index phải được reset.
    assert list(
        result.index
    ) == list(
        range(len(result))
...
def test_filter_protocol_invalid_none_khong_loai_du_lieu(
    ...
    """
    Nếu excluded_persons=None thì phải giữ nguyên số sample.
    """
...
def test_filter_protocol_invalid_bao_loi_khi_loai_het_dataset(
    ...
    """
    Nếu cấu hình loại toàn bộ person thì phải báo lỗi.
    """
...
def test_prepare_dataset_end_to_end(
    ...
    Đây là test gần nhất với cách train.py sẽ sử dụng preprocessing.py.
    """
    ...
    # Chỉ còn person01.
    assert set(
        prepared.groups.unique()
    ) == {"person01"}

    # person05 phải được loại hoàn toàn.
    assert (
        "person05"
        not in set(
            prepared.df["person_id"]
        )
    )

    # Vì fixture có 4 class cho person01,
    # sau filter vẫn còn đủ 4 labels.
    assert set(
        prepared.df["label"]
    ) == set(
        prep.EXPECTED_CLASSES
    )

    # X vẫn phải có đúng 29 features.
    assert (
        prepared.X.shape[1]
        == prep.EXPECTED_NUM_FEATURES
    )

    # Số sample của các output phải giống nhau.
    assert (
        len(prepared.X)
        == len(prepared.y)
        == len(prepared.groups)
...
def test_prepare_dataset_khong_filter_person_khi_excluded_rong(
    ...
    """
    Nếu không truyền person cần loại,
    toàn bộ dataset hợp lệ phải được giữ lại.
    """
